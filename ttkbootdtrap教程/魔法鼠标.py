...
class MouseUtilities(ttk.Frame):
    ...
    def __init__(self, master):
        ...
        for i in range(3):
            self.columnconfigure(i, weight=1)#法是用来配置网格布局管理器
        ...
        # device info
        dev_info = ttk.Labelframe(col1, text='Device Info', padding=10) #标签窗口
        ...
        # progressbar
        pb = ttk.Progressbar(dev_info, value=66)     #进度条
        ...
        lic_info.rowconfigure(0, weight=1)#行
        """rowconfigure方法用于配置网格布局管理器（grid geometry manager）中行的行为。
        当您使用grid布局来放置控件时，rowconfigure和columnconfigure方法允许您指定如何分配额外的空间到行和列中"""
        lic_info.columnconfigure(0, weight=2)#列
        ...
        op1 = ttk.Checkbutton(scrolling, text='Scrolling', variable='op1') #多选框
        ...
        scale = ttk.Scale(scroll_speed_frame, value=35, from_=1, to=100)#水银计
