...
class BackMeUp(ttk.Frame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.photoimages = []   #设置一个空列表
        imgpath = Path(__file__).parent / 'assets' #创建一个新文件路径
        # __file__：这是一个内置变量，它表示当前执行的脚本文件的路径）。
        # Path(__file__)：这里，我们使用 Path 类来创建一个 Path 对象。
        # .parent：这是 Path 对象的一个属性，它返回该路径的父目录。
        # / 'assets'：这里，它将父目录路径与 'assets' 字符串连接起来，形成一个新的路径

        for key, val in image_files.items(): #获取image_files字典的建值对
            _path = imgpath / val  #获取文件的绝对路径
            self.photoimages.append(ttk.PhotoImage(name=key, file=_path))  #王上面的空列表中添加数据

        # buttonbar
        buttonbar = ttk.Frame(self, style='primary.TFrame')  #创建一个新的页面
        ...
        self.setvar('destination', 'd:/test/')    #从继承中获取的一个函数
        ...
        _func = lambda: Messagebox.ok(message='Adding to backup')       #调用匿名函数，
        add_btn = ttk.Button(      #创建按钮
            master=bus_frm,
            text='Add to backup',
        ...
        status_cf = CollapsingFrame(left_panel)     #调用CollapsingFrame类
        status_cf.pack(fill=BOTH, pady=1)

        ## container
        status_frm = ttk.Frame(status_cf, padding=10)
        status_frm.columnconfigure(1, weight=1)    #设置列的相对权重、最小和最大尺寸等属性
        status_cf.add(          #调用CollapsingFrame类中的add函数
            child=status_frm,
            title='Backup Status',
            bootstyle=SECONDARY
        )
        ...
        ## progress bar
        pb = ttk.Progressbar(       #创建进度条（ProgressBar）
            master=status_frm,
            variable='prog-value',
            bootstyle=SUCCESS
        )
        ...
        ## section separator在用户界面上创建分隔线的控件
        ...
        ## Treeview
        tv = ttk.Treeview(right_panel, show='headings', height=5)      #图形用户界面（GUI）中显示树形结构的数据
        tv.configure(columns=(                        #对tv中的colums进行重新定义
        ...
        st = ScrolledText(output_container)# 带滚动条的文本框
